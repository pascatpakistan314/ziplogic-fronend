...
export default function ProjectEditor() {
  ...
  const startPreview = async () => {
    ...
    try {
      // Use Stackblitz SDK — no backend preview API needed
      ...
      let parsedPkg = {}
      try {
        if (packageJson) parsedPkg = JSON.parse(packageJson)
      } catch { }

      // ═══════════════════════════════════════════════════════
      // FIX: Ensure we have file contents for every path
      // ═══════════════════════════════════════════════════════
      const filesWithContent = { ...files }
      const pathsMissing = Object.keys(filesWithContent).filter(p => 
        filesWithContent[p] === null || 
        filesWithContent[p] === undefined || 
        filesWithContent[p] === ''
      )
      
      // Load missing file contents
      for (const p of pathsMissing) {
        try {
          const res = await projectsAPI.getFileContent(id, p)
          filesWithContent[p] = res.data.content || ''
        } catch (err) {
          console.warn(`Failed to load ${p}, using stub`)
          // Create stub based on file type
          if (p.endsWith('.json')) {
            filesWithContent[p] = '{}'
          } else if (p.endsWith(('.ts', '.tsx', '.js', '.jsx'))) {
            filesWithContent[p] = '// File content not available\nexport default function Component() { return null; }'
          } else if (p.endsWith('.css')) {
            filesWithContent[p] = '/* Styles */'
          } else {
            filesWithContent[p] = ''
          }
        }
      ...
      // ═══════════════════════════════════════════════════════
      if (!filesWithContent['package.json']) {
        console.warn('package.json missing, creating default')
        filesWithContent['package.json'] = JSON.stringify({
          name: 'ziplogic-project',
          version: '0.1.0',
          private: true,
          scripts: {
            dev: 'next dev',
      ...
      const project = {
        title: parsedPkg.name || project?.name || 'ZipLogic Project',
        description: 'Generated by ZipLogic AI',
        template: 'node',
      ...
      if (stackblitzContainerRef.current) {
        ...
        setPreviewUrl('stackblitz-embedded')
        toast.success('Preview started!')
      }
    } catch (err) {
      console.error('Stackblitz preview error:', err)
      toast.error('Preview start failed: ' + (err.message || 'Unknown error'))
    } finally {
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            {showPreview && (
              <div className="w-[45%] flex-shrink-0 border-l border-white/10 flex flex-col">
                ...
                {previewUrl ? (
                  <div ref={stackblitzContainerRef} className="flex-1 bg-white" style={{ border: 'none' }} />
                ) : (
